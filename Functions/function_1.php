...
function showRoundRobin($parm = '') {
    ...
    foreach (json_decode($parm) as $value) {
        ...
        if(like_match('%proceso%',$value->name)){ //in_array($value->name, array("txtProceso1","txtProceso2","txtProceso3","txtProceso4","txtProceso5"))){
        ...
        if(like_match('%numero%',$value->name)){ //in_array($value->name, array("txtProceso1","txtProceso2","txtProceso3","txtProceso4","txtProceso5"))){            
            if(isset($value->value) && !empty($value->value)){  
    ...
    // Ejecutamos los procesos en orden cíclico
    while (true) {
      $proceso_completo = true;

      // Iteramos sobre cada proceso
      for ($i = 0; $i < count($procesos); $i++) {
        ...
        if ($tiempos_restantes[$i] > 0) {
          ...
          if ($tiempos_restantes[$i] > $quantum) {
            // Ejecutamos el proceso por el quantum asignado
            $tiempo_total += $quantum;
            $tiempos_restantes[$i] -= $quantum;
          } else {
            // Ejecutamos el proceso por el tiempo restante y lo marcamos como completo
